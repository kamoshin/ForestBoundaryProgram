...
class_list = ['japanese_cedar', 'japanese_cypress', 'other_tree', 'not_tree']
...
save_dir = './../../../../Data/pred_value/weight_adjust/test/'
    ...
    for r_y in range(0, height, slide):
        ...
        for r_x in range(0, width, slide):
            ...
            if not all(colormap_label[r_y][r_x] == [0, 0, 0, 0]):
                pred_value_set = []
                pred_value_set.append(learge_pred[r_y][r_x][0])
                pred_value_set.append(middle_pred[r_y][r_x][0])
                pred_value_set.append(small_pred[r_y][r_x][0])
                pred_value_set.append(most_small_pred[r_y][r_x][0])
                if not any(np.isnan(pred_value_set)):
                    X.append(pred_value_set)
                    Y.append(colormap_label[r_y][r_x][0])
    X = np.asarray(X)
    Y = np.asarray(Y)
    np.save(save_dir+'X/'+class_list[0]+'/'+'X_'+class_list[0]+'_'+str(image_n)+str(lerge_size)+'_'+str(middle_size)+'_'+str(small_size)+'_'+str(most_small_size), X)
    np.save(save_dir+'Y/'+class_list[0]+'/'+'Y_'+class_list[0]+'_'+str(image_n)+str(lerge_size)+'_'+str(middle_size)+'_'+str(small_size)+'_'+str(most_small_size), Y)
print('end')
